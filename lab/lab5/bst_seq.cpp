    ...
    *R = NULL;
	...
	if (r <= ror) {
		...
		*R = T;
		...
		split(T->right, r-ror-1, &T->right, R);
		*L = T;
	}
	fix_size(T);
}
...
Node *insert_keep_balanced(Node *T, int v, int r)
{
  ...
  int ror = T->left ? T->left->size : 0;
  if (r <= ror) {
	  T->left = insert_keep_balanced(T->left, v, r);
  }
  else {
	  T->right = insert_keep_balanced(T->right, v, r-ror-1);
  }
  fix_size(T);
  return T;
}
...
Node *order_n_teams(int n)
{
  ...
  Node *temp = T;
	while (b<e){
    r = ((b+e)/2);
		if (did_x_beat_y(i, select(temp,r)->key)) {
			r = temp->right ? temp->right->size : 0;
			e= r-1;
			temp= temp->left;
		}
		else {
			r = temp->left ? temp->left->size : 0;
			b = temp->size-r;
			temp= temp->right;
      //cout << b << "  " << e << endl;
		}
