...
Node *select(Node *T, int r)
{
  ...
  int rankR = T->left->size;
  if (r == rankR) return T;
  if (r < rankR) return select(T->left, r);
  else
	  return select(T->right, r - rankR - 1);
}
...
{
	if (T == NULL) return new Node(k);
	//with prob 1 out of (T->size+1), insert at root...
	if (rand() % (T->size+1) == 0) {
		Node *new_root = new Node(k);
		new_root->size = T->size + 1;
		split(T, k, &new_root->left, &new_root->right);
		//new_root->size += new_root->left == NULL ? 0 : new_root->left->size;
		//new_root->size += new_root->right == NULL ? 0 :new_root->right->size;
		return new_root;
	} else {
